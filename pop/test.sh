...
testOlStore(){
  ...
  PARTYID=$SED
  testOK test -n "$PARTYID"
  testFail runCl 1 ol coin show $OL $PARTYID ${pub[1]}
  testFail runCl 1 ol finalize $OL $KEY ${pop_hash[1]} $PARTYID
  ...
  for (( o=1; o<=3; o++ )); do
	...
	testOK runCl 1 ol finalize $OL $KEY ${pop_hash[1]} $PARTYID
...
mkPopConfig(){
  ...
  for (( p=1; p<=$1; p++ ))
  do
    rm -f pop_desc$p.toml
    cat << EOF > pop_desc$p.toml
Name = "Proof-of-Personhood Party"
DateTime = "2017-08-08 15:00 UTC"
Location = "Earth, City$p"
EOF
    for (( o=1; o<=$2; o++ ))
    do
      cat co$o/public.toml >> pop_desc$p.toml
    done
  done
  rm -f pop_merge.toml
  for (( o=1; o<=$2; o++ ))
...
EOF
    ...
    tail -n +2 co$o/public.toml >> pop_merge.toml
    local m=$(($o%$2 + 1))
    echo "[[parties.servers]]" >> pop_merge.toml
