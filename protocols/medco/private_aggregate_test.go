...
func TestPrivateAggregate(t *testing.T) {
	...
	_ /*entityList*/, _, tree := local.GenTree(10, true)
	...
	//verify results
	expectedGroups := map[libmedco.GroupingKey]libmedco.GroupingAttributes{groupingAttrA.Key(): groupingAttrA,
		groupingAttrB.Key(): groupingAttrB,
		groupingAttrC.Key(): groupingAttrC}

	expectedResults := map[libmedco.GroupingKey][]int64{groupingAttrA.Key(): {3, 5, 7, 9, 11},
	...
	case encryptedResult := <-feedback:
		...
		resultData := make(map[libmedco.GroupingKey][]int64)
		for k, v := range encryptedResult.GroupedData {
			resultData[k] = libmedco.DecryptIntVector(clientPrivate, &v)
...
func NewPrivateAggregateTest(tni *sda.TreeNodeInstance) (sda.ProtocolInstance, error) {
	...
	case 0:
		...
		testCVMap[groupingAttrA.Key()] = *libmedco.EncryptIntVector(clientPublic, []int64{1, 2, 3, 4, 5})
		testGAMap[groupingAttrB.Key()] = groupingAttrB
		testCVMap[groupingAttrB.Key()] = *libmedco.EncryptIntVector(clientPublic, []int64{0, 0, 0, 0, 0})
	case 1:
		testGAMap[groupingAttrB.Key()] = groupingAttrB
		testCVMap[groupingAttrB.Key()] = *libmedco.EncryptIntVector(clientPublic, []int64{1, 2, 3, 4, 5})
	case 2:
		testGAMap[groupingAttrA.Key()] = groupingAttrA
		testCVMap[groupingAttrA.Key()] = *libmedco.EncryptIntVector(clientPublic, []int64{1, 1, 1, 1, 1})
	case 9:
		testGAMap[groupingAttrC.Key()] = groupingAttrC
		testCVMap[groupingAttrC.Key()] = *libmedco.EncryptIntVector(clientPublic, []int64{1, 0, 1, 0, 1})
		testGAMap[groupingAttrA.Key()] = groupingAttrA
		testCVMap[groupingAttrA.Key()] = *libmedco.EncryptIntVector(clientPublic, []int64{1, 2, 3, 4, 5})
	case 5:
		testGAMap[groupingAttrC.Key()] = groupingAttrC
		testCVMap[groupingAttrC.Key()] = *libmedco.EncryptIntVector(clientPublic, []int64{0, 1, 0, 1, 0})
	default:
	}
