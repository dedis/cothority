...
func NewNtreeProtocol(node *sda.Node) (*Ntree, error) {
	...
	node.RegisterChannel(&nt.commitChan)
	node.RegisterChannel(&nt.signatureRequestChan)
	node.RegisterChannel(&nt.signatureResponseChan)
...
func NewNTreeRootProtocol(node *sda.Node, transactions []blkparser.Tx) (*Ntree, error) {
	...
	nt.block = getBlock(transactions, "", "")
	return nt, nil
}
...
func (nt *Ntree) Start() error {
	...
	for _, tn := range nt.Children() {
		nt.SendTo(tn, &BlockAnnounce{tn.block})
	}
}
...
func (nt *Ntree) listen() {
	for {
		...
		case msg := <-nt.blockSignatureChan:
			nt.handleBlockSignature(&msg.BlockSignature)
...
func (nt *Ntree) computeBlockSignature() {
	...
	if !ok {
		...
		schnorr, err := crypto.SchnorrSig(nt.Suite(), nt.Private(), marshalled)
...
func (nt *Ntree) handleBlockSignature(msg BlockSignature) {
...
func (nt *Ntree) startSignatureRequest(msg *BlockSignature) {
...
func (nt *Ntree) verifySignatureRequest(msg RoundSignatureRequest) {
	...
	for _, sig := range msg.Sigs {
		if err := crypto.SchnorrVerify(nt.Suite(), nt.Public(), nt.block, sig); err == nil {
			goodSig++
		}
		sigBuf, _ = sig.MarshalBinary()
	}

	// putting the header of the block as the message to sign
	nt.signatureBuffer, _ = json.Marshal(nt.block)
...
func (nt *Ntree) computeSignatureResponse() {
	...
	if !ok {
		...
		sig, err := crypto.SchnorrSig(nt.Suite(), nt.Private(), marshalled)
		if err != nil {
			return err
		}
