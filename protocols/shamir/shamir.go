...
func certifyMatrix(matrix []*JointPromiser) {
	...
	fmt.Printf("[+] All Peers successfully CERTIFIED their promises !\n")
}

// Produce the master pub polynomial which consists off adding off each PubPoly's peer
func produceMasterPublicPoly(matrix []*JointPromiser) {
	for i, jp := range matrix {
		master := jp.PeerDatas[0].Promise.PubPoly()
		// range over the promises of each peers
		for _, peerData := range jp.PeerDatas[1:] {
			// Add homomorphically the different pubpoly to produce the master
			master = master.Add(master, peerData.Promise.PubPoly())
		}
		matrix[i].MasterPubPoly = master
	}
	// verify that we get the same Master PubPoly for each one
	for i, jp := range matrix {
		...
		for j, jjp := range matrix {
			...
			if !jp.MasterPubPoly.Equal(jjp.MasterPubPoly) {
				fmt.Printf("[-] Error while checking the Master PubPoly for peer %d vs %d : Not Equals ...\n", i, j)
				os.Exit(1)
			}
		}
	}

	fmt.Printf("[+] Master PubPoly successfully created and verified amongst each peer !")
}
...
func jointShamirPromiseTest() {
	...
	produceMasterPublicPoly(matrix)
