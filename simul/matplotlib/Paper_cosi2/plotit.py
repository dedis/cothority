...
def plotAvgMM(cosi1, jvss = None, naive = None, nt = None):
    mplot.plotPrepareLogLog()
    ...
    xmin, xmax, ymin, ymax = CSVStats.get_min_max(co)
    plt.ylim(ymin, 8)
...
def plotCoSiOld(cosi_old, cosi_3, cosi_4, cosi_5):
    ...
    co_old_depth = []
    for x in range(0, len(co_old_hosts)):
        bf = co_old_bf[x]
        hosts = co_old_hosts[x]
        co_old_depth.append(0)
        for depth in range(0, 10):
            h = (1 - (bf ** depth)) / (1 - bf)
            if h <= hosts:
                co_old_depth[x] = depth
    print cosi_old.x
    print co_old_hosts
    print co_old_bf
    print co_old_depth
...
def PlotMultiBF(*values_bf):
    ...
    for i, value in enumerate(values_bf):
        label = str(int(value.columns['hosts'][0])) + " hosts"
        c1, c2, m = pparams[i]
...
def PlotStamp(stamp):
    ...
    plotbf = mplot.plotMMA(stamp, 'round_wall', color1_light, 0,
                           dict(label='4096 hosts', linestyle='-', marker='o', color=color2_dark, zorder=3))
...
def args_to_csv(argn, xname = "hosts"):
    stats = []
