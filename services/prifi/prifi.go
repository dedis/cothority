...
func init() {
	sda.RegisterNewService(ServiceName, newPrifiService)
}

func newPrifiService(c sda.Context, path string) sda.Service {
	return &PriFi{
		c:    c,
		path: path,
	}
...
func (p *PriFi) ProcessClientRequest(e *network.Entity, r *sda.ClientRequest) {
...
func (p *PriFi) ProcessServiceMessage(e *network.Entity, s *sda.ServiceMessage) {
	return
}
