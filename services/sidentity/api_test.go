...
func NewTestIdentity(cothority *sda.Roster, majority int, owner string, pinstate *common_structs.PinState, cas []common_structs.CAInfo, local *sda.LocalTest) *Identity {
	id := NewIdentity(cothority, majority, owner, pinstate, cas)
	id.CothorityClient = local.NewClient(ServiceName)
	return id
}

func NewTestIdentityMultDevs(cothority *sda.Roster, majority int, owners []string, pinstate *common_structs.PinState, cas []common_structs.CAInfo, local *sda.LocalTest) []*Identity {
