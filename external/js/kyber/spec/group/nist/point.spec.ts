...
describe("Nist Point", () => {
    ...
    // prettier-ignore
    const marshalled = Buffer.from([4, 86, 136, 95, 219, 46, 44, 21, 129, 228, 251, 109, 189, 14, 233, 39, 200,
        61, 230, 250, 80, 93, 166, 150, 168, 69, 80, 207, 81, 252, 111, 247, 159, 50, 200, 1, 128, 38, 107, 124, 61, 43,
        130, 195, 203, 232, 91, 139, 88, 232, 48, 229, 188, 47, 236, 127, 85, 30, 29, 69, 170, 227, 163, 245, 197]);
    ...
    it("should print the string representation of a point", () => {
        ...
        // prettier-ignore
        const target = "(39139857753964535406422970543512609321558395110412588924902544519776250623903," +
            "22969022198784600445029639705880320580068058102470723316833310869386179114437)";
        point.unmarshalBinary(marshalled);
    ...
    it("should throw an exception on an invalid point", () => {
        let b = Buffer.from(marshalled);
        b[1] = 11;
        expect(() => curve.point().unmarshalBinary(b)).toThrow();

        b = Buffer.from(marshalled);
        b[0] = 5;
        expect(() => curve.point().unmarshalBinary(b)).toThrow();
    });
    ...
    it("should marshal the point according to spec", () => {
        ...
        expect(point.marshalBinary()).toEqual(marshalled);
    });
    ...
    it("should embed data with length = embedLen", () => {
        // prettier-ignore
        const data = Buffer.from([68, 69, 68, 73, 83, 68, 69, 68, 73, 83, 68, 69, 68, 73, 83, 68, 69, 68, 73,
            83, 68, 69, 68, 73, 83, 68, 69, 68, 73, 83]);
        const point = curve.point().embed(data, prng.pseudoRandomBytes);
    ...
    it("should throw an Error on embeded length > embedLen", () => {
        ...
        // prettier-ignore
        const bytes = Buffer.from([4, 201, 209, 147, 190, 134, 219, 80, 165, 6, 231, 153, 126, 240, 204, 175,
            212, 170, 3, 0, 156, 228, 220, 14, 189, 212, 105, 250, 84, 26, 5, 195, 137, 6, 162, 237, 154, 18, 5, 159,
            120, 82, 140, 135, 94, 18, 162, 95, 112, 39, 108, 199, 167, 17, 65, 78, 9, 156, 173, 246, 10, 104, 224, 192,
            157]);
        const point = curve.point();
        point.unmarshalBinary(bytes);
