...
describe("Nist Scalar", () => {
    ...
    it("should add two scalars", () => {
        ...
        // prettier-ignore
        const target = Buffer.from([190, 106, 201, 42, 29, 98, 50, 20, 33, 0, 19, 67, 158, 237, 104, 224, 224,
            253, 31, 149, 82, 182, 97, 87, 34, 5, 98, 17, 87, 61, 172, 180]);
    ...
    it("should subtract two scalars", () => {
        const sb1 = Buffer.from([1, 2, 3, 4]);
        const sb2 = Buffer.from([5, 6, 7, 8]);
        const s1 = curve.scalar().setBytes(sb1);
        const s2 = curve.scalar().setBytes(sb2);
        const diff = curve.scalar().sub(s1, s2);
        // prettier-ignore
        const target = Buffer.from([255, 255, 255, 255, 0, 0, 0, 0, 255, 255, 255, 255, 255, 255, 255, 255,
            188, 230, 250, 173, 167, 23, 158, 132, 243, 185, 202, 194, 248, 95, 33, 77]);
        const s3 = curve.scalar().setBytes(target);
    ...
    it("should negate a point", () => {
        ...
        // prettier-ignore
        const target = Buffer.from([154, 39, 145, 231, 128, 248, 52, 6, 49, 85, 200, 164, 158, 16, 33, 96, 146,
            236, 120, 242, 154, 155, 254, 225, 166, 156, 209, 20, 34, 240, 40, 166]);
    ...
    it("should multiply two scalars", () => {
        ...
        // prettier-ignore
        const target = Buffer.from([88, 150, 22, 208, 89, 155, 151, 255, 177, 162, 187, 27, 200, 24, 106, 226,
            148, 44, 50, 249, 104, 23, 185, 233, 226, 79, 51, 233, 132, 194, 166, 138]);
        const s3 = curve.scalar().setBytes(target);
    ...
    it("should divide two scalars", () => {
        ...
        // prettier-ignore
        const target = Buffer.from([197, 214, 67, 20, 213, 10, 109, 3, 187, 62, 94, 90, 111, 152, 254, 126,
            57, 162, 144, 250, 104, 92, 124, 206, 143, 31, 20, 64, 4, 243, 185, 241]);
        const s3 = curve.scalar().setBytes(target);
    ...
    it("should compute the inverse modulo n of scalar", () => {
        ...
        // prettier-ignore
        const target = Buffer.from([65, 112, 236, 29, 4, 150, 6, 224, 144, 13, 175, 197, 232, 73, 19, 137,
            150, 235, 201, 127, 55, 45, 109, 196, 104, 154, 215, 9, 171, 186, 177, 58]);
        const s2 = curve.scalar().setBytes(target);
    ...
    it("should pick a random scalar", () => {
        ...
        // prettier-ignore
        const bytes = Buffer.from([225, 208, 244, 196, 143, 183, 151, 13, 179, 199, 181, 81, 189, 241, 253,
            227, 46, 34, 167, 212, 41, 112, 79, 126, 170, 10, 139, 193, 110, 187, 30, 231]);
