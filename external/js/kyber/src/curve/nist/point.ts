...
export default class NistPoint implements Point {
    ...
    embed(data: Buffer, callback?: (length: number) => Buffer): NistPoint {
        ...
        while (true) {
            ...
            const buff = callback(buffLen);
            const highbits = bitLen & 7;
            if (highbits !== 0) {
    ...
    data(): Buffer {
        const l = this.ref.curve.coordLen();
        const b = Buffer.from(this.ref.point.x.fromRed().toArray("be", l));
        const dl = b[l - 1];
        if (dl > this.embedLen()) {
    ...
    marshalBinary(): Buffer {
        ...
        const xBytes = Buffer.from(this.ref.point.x.fromRed().toArray("be"));
        xBytes.copy(buf, 1 + byteLen - xBytes.length);
        const yBytes = Buffer.from(this.ref.point.y.fromRed().toArray("be"));
    ...
    unmarshalBinary(bytes: Buffer): void {
        ...
        const x = new BN(bytes.slice(1, 1 + byteLen), 16);
        const y = new BN(bytes.slice(1 + byteLen), 16);
