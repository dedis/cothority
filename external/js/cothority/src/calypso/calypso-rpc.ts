...
export class OnChainSecretRPC {
  ...
  constructor(public bc: ByzCoinRPC, roster?: Roster) {
    this.socket = new RosterWSConnection(
      bc.getConfig().roster,
      OnChainSecretRPC.serviceID
    );
    if (roster) {
  ...
  async createLTS(
    ...
    darcID: InstanceID,
    signers: Signer[]
  ): Promise<CreateLTSReply> {
    const buf = Buffer.from(
      LtsInstanceInfo.encode(new LtsInstanceInfo({ roster: r })).finish()
    );
  ...
  async authorize(
    ...
    const sock = new WebSocketConnection(
      who.getWebSocketAddress(),
      OnChainSecretRPC.serviceID
    );
  ...
  async reencryptKey(write: Proof, read: Proof): Promise<DecryptKeyReply> {
    const sock = new WebSocketConnection(
      this.list[0].getWebSocketAddress(),
      OnChainSecretRPC.serviceID
    );
