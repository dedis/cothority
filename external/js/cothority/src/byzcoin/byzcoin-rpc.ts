...
import { ClientTransaction, InstanceID } from "./ClientTransaction";
import { DarcInstance } from "./contracts/DarcInstance";
import { Identity } from "../darc/Identity";
import { Connection, RosterWSConnection } from "../network/connection";
...
export default class ByzCoinRPC {
    ...
    async updateConfig(): Promise<void> {
        let configIID = new InstanceID(Buffer.alloc(32, 0));
    ...
    async getSignerCounters(ids: Identity[], add: number = 0): Promise<number[]> {
    ...
    static makeGenesisDarc(signers: Identity[], roster: Roster): Darc {
        if (signers.length == 0) {
            throw new Error("no identities");
        }

        const d = Darc.newDarc(signers, signers, Buffer.from("Genesis darc"));
    ...
    static async fromByzcoin(roster: Roster, id: Buffer): Promise<ByzCoinRPC> {
        ...
        const ccProof = await rpc.getProof(new InstanceID(Buffer.alloc(32, 0)));
        rpc.config = ChainConfig.fromProof(ccProof);

        const gdProof = await rpc.getProof(new InstanceID(ccProof.stateChangeBody.darcID));
        rpc.genesisDarc = DarcInstance.darcFromProof(gdProof);
