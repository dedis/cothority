...
func init() {
	onet.GlobalProtocolRegister(NameOCSNT, NewOCSNT)
}
...
func NewOCSNT(n *onet.TreeNodeInstance) (onet.ProtocolInstance, error) {
	...
	err := o.RegisterHandlers(o.partialReencrypt)
	if err != nil {
...
// the share
func (o *OCSNT) partialReencrypt(pr structPartialReencrypt) error {
	log.Lvl3(o.Name() + ": starting partialreencrypt")
	defer o.Done()

	ui := o.getUI(pr.U, pr.Xc)

	if o.Verify != nil {
		if !o.Verify(&pr.PartialReencrypt) {
	...
	uiHat := cothority.Suite.Point().Mul(si, cothority.Suite.Point().Add(pr.U, pr.Xc))
