...
// HasRole returns true if the Identity exists in the owners or users, depending
// on the given role.
func (d *Darc) HasRole(identity *Identity, role Role) (found bool) {
	var list *[]*Identity
	if role == Owner {
		list = d.Owners
	} else {
...
// It returns a signer.
func NewSignerEd25519(secret kyber.Scalar) *Signer {
	var point kyber.Point
	if secret == nil {
		kp := key.NewKeyPair(cothority.Suite)
		point, secret = kp.Public, kp.Private
	} else {
		point = cothority.Suite.Point().Mul(secret, nil)
	}
