...
type Node struct {
	...
	nRounds             int
	RemoveMerkle        map[int]*RoundMerkle
...
func NewNode(hn coconet.Host, suite abstract.Suite, random cipher.Stream) *Node {
	...
	sn.RemoveMerkle = make(map[int]*RoundMerkle)
...
func (sn *Node) SetAccountableRound(Round int) {
	...
	h.Write(sn.RemoveMerkle[Round].BackLink)
	sn.RemoveMerkle[Round].AccRound = h.Sum(nil)
...
func (sn *Node) SetBackLink(Round int) {
	prevRound := Round - 1
	sn.RemoveMerkle[Round].BackLink = hashid.HashId(make([]byte, hashid.Size))
	if prevRound >= FIRST_ROUND {
		// My Backlink = Hash(prevRound, sn.Rounds[prevRound].BackLink, sn.Rounds[prevRound].MTRoot)
		h := sn.suite.Hash()
		if sn.RemoveMerkle[prevRound] == nil {
			dbg.Lvl1(sn.Name(), "not setting back link")
			return
		}
		h.Write(intToByteSlice(prevRound))
		h.Write(sn.RemoveMerkle[prevRound].BackLink)
		h.Write(sn.RemoveMerkle[prevRound].MTRoot)
		sn.RemoveMerkle[Round].BackLink = h.Sum(nil)
	}
}
