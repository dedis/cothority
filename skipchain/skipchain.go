...
func (s *Service) StoreSkipBlock(psbd *StoreSkipBlock) (*StoreSkipBlockReply, error) {
	...
	if psbd.LatestID.IsNull() && psbd.NewBlock.GenesisID.IsNull() {
		...
		if prev == nil {
			...
			// If we know of this chain, try to sync it. First get latest from database,
			// ignore errors - we try as hard as we can.
			latest, _ := s.db.GetLatest(gen)
		...
		var err error
		prev, err = s.db.GetLatest(prev)
		if err != nil {
			return nil, errors.New("error while trying to find latest block: " + err.Error())
		}
