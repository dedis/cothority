...
func TestService_ProposeSkipBlock(t *testing.T) {
	...
	assert.Nil(t, err)
	psbr := psbrMsg.(*StoreSkipBlockReply)
	latest := psbr.Latest
	...
	id := psbr.Latest.Hash
	psbrMsg, err = service.StoreSkipBlock(&StoreSkipBlock{id, next})
	assert.Nil(t, err)
	psbr2 := psbrMsg.(*StoreSkipBlockReply)
	log.Lvl2(psbr2)
	if psbr2 == nil {
...
func TestService_GetUpdateChain(t *testing.T) {
	...
	for i := 1; i < sbCount; i++ {
		...
		psbrMsg, err := service.StoreSkipBlock(&StoreSkipBlock{sbs[i-1].Hash, newSB})
		assert.Nil(t, err)
		reply := psbrMsg.(*StoreSkipBlockReply)
	...
	for i := 0; i < sbCount; i++ {
		...
		log.Printf("Update: %s, Full: %s", sbc.Update, sbs)
	...
	for base := 1; base <= 3; base++ {
		for height := 1; height <= base; height++ {
			...
			for sbi := 1; sbi < 10; sbi++ {
				...
				log.ErrFatal(err)
				latest = psbr.(*StoreSkipBlockReply).Latest
			}
...
func TestService_Verification(t *testing.T) {
	...
	sb.VerifierIDs = []VerifierID{VerifyShard}
...
func TestService_SignBlock(t *testing.T) {
	...
	sb := NewSkipBlock()
	sb.Roster = el2
	psbr, err := service.StoreSkipBlock(&StoreSkipBlock{sbRoot.Hash, sb})
	log.ErrFatal(err)
	reply := psbr.(*StoreSkipBlockReply)
...
func makeGenesisRosterArgs(s *Service, el *onet.Roster, parent SkipBlockID,
	...
	sb.VerifierIDs = vid
	psbrMsg, err := s.StoreSkipBlock(&StoreSkipBlock{nil, sb})
	if err != nil {
		return nil, err
	}
	psbr := psbrMsg.(*StoreSkipBlockReply)
