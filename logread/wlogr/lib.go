...
type wlrConfig struct {
	ACLBunch *skipchain.SkipBlockBunch
	WLRBunch *skipchain.SkipBlockBunch
...
// tests.
func CreateWLRBunch(roster *onet.Roster, sid skipchain.SkipBlockID) (*skipchain.SkipBlockBunch, error) {
	cl := skipchain.NewClient()
	sbs, err := cl.GetBlocks(roster, sid, nil, 0)
	if err != nil {
		return nil, err
	}
	if len(sbs) == 0 {
...
// tests.
func CreateACLBunch(roster *onet.Roster, sid skipchain.SkipBlockID) (*skipchain.SkipBlockBunch, error) {
	cl := skipchain.NewClient()
	sbs, err := cl.GetBlocks(roster, sid, nil, 0)
	if err != nil {
		return nil, err
